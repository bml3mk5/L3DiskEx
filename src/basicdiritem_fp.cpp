...
DiskBasicDirItemFP::DiskBasicDirItemFP(DiskBasic *basic, DiskD88Sector *sector, int secpos, wxUint8 *data)
	: DiskBasicDirItemFAT8(basic, sector, secpos, data)
{
}
DiskBasicDirItemFP::DiskBasicDirItemFP(DiskBasic *basic, int num, int track, int side, DiskD88Sector *sector, int secpos, wxUint8 *data, bool &unuse)
	: DiskBasicDirItemFAT8(basic, num, track, side, sector, secpos, data, unuse)
{
	Used(CheckUsed(unuse));
	unuse = (unuse || (m_data->fp.name[0] == 0xff));
...
{
	if (num == 0) {
		size = len = sizeof(m_data->fp.name);
		return m_data->fp.name;
	} else {
		size = len = 0;
		return NULL; 
	}
...
bool DiskBasicDirItemFP::Check(bool &last)
{
	...
	// ディレクトリが不正
	if (((m_data->fp.type & 2) != 0) || (m_data->fp.term != 0 && m_data->fp.term != 0xff)) {
		valid = false;
	}
	return valid;
}
...
void DiskBasicDirItemFP::SetTerminate(int val)
{
	...
	int c;
	c = (val != 0 ? 0xff : 0);
	m_data->fp.term = (c & 0xff);

	c = ((val & FILETYPE_FP_BASIC) != 0 ? 0xff : 0);
	memset(m_data->fp.unknown, c, sizeof(m_data->fp.unknown));
}
...
void DiskBasicDirItemFP::SetFileSize(int val)
{
	...
	if (t1 == FILETYPE_FP_BASIC) {
		// BASIC - binary
		m_data->fp.file_size = wxUINT16_SWAP_ON_BE(val);
		m_data->fp.end_addr = wxUINT16_SWAP_ON_BE(val);
	} else if (t1 == FILETYPE_FP_MACHINE) {
		// Machine
		m_data->fp.file_size = wxUINT16_SWAP_ON_BE(val);
		val = wxUINT16_SWAP_ON_BE(m_data->fp.load_addr) + val - 1;
		m_data->fp.end_addr = wxUINT16_SWAP_ON_BE(val);
	}
...
{
	int val = wxUINT16_SWAP_ON_BE(m_data->fp.file_size);
	if (val == 0) {
...
{
	m_data->fp.load_addr = wxUINT16_SWAP_ON_BE(val);
}
...
{
	m_data->fp.exec_addr = wxUINT16_SWAP_ON_BE(val);
}
...
{
	// fp
	m_data->fp.start_group = (val & 0xff);
}
...
{
	// fp
	return m_data->fp.start_group;
}

/// 削除
bool DiskBasicDirItemFP::Delete(wxUint8 code)
{
	ClearData();
	Used(false);
	return true;
