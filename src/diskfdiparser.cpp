...
//
//
DiskFDIParser::DiskFDIParser(DiskD88File *file, short mod_flags, DiskResult *result)
...
int DiskFDIParser::Check(DiskParser &dp, wxInputStream &istream, const wxArrayString *disk_hints, const DiskParam *disk_param, DiskParamPtrs &disk_params, DiskParam &manual_param)
{
	...
	// ディスクテンプレートから探す
	DiskParticulars sd, pt;
	const DiskParam *param = gDiskTemplates.FindStrict(sides_per_disk, tracks_per_side, sectors_per_track, sector_size, 1, 0, sd, pt);
	if (param) {
