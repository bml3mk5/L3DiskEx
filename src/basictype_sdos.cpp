...
/// @retval  0 : 終了しない
/// @retval -1 : 強制的に終了
int DiskBasicTypeSDOS::FinishAssigningDirectory(int size) const
{
	// サイズに達したら終了
	return (size < (int)sizeof(directory_sdos_t) ? -1 : 0);
}
...
void DiskBasicTypeSDOS::CalcDiskFreeSize(bool wrote)
{
	...
	for(wxUint32 pos = 0; pos <= basic->GetFatEndGroup(); pos++) {
		if (pos < gend) {
			...
			if (pos < max_gnum) {
				// 削除されたエリア
				fat_availability.Item(pos) = FAT_AVAIL_LEAK;
			} else {
				// 空き
				grps++;
			}
...
bool DiskBasicTypeSDOS::PrepareToSaveFile(wxInputStream &istream, int &file_size, const DiskBasicDirItem *pitem, DiskBasicDirItem *nitem, DiskBasicError &errinfo)
{
	return true;
}
...
/// @return 書き込んだバイト数
int DiskBasicTypeSDOS::WriteFile(DiskBasicDirItem *item, wxInputStream &istream, wxUint8 *buffer, int size, int remain, int sector_num, wxUint32 group_num, wxUint32 next_group, int sector_end)
{
