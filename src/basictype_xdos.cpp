...
double DiskBasicTypeXDOS::ParseParamOnDisk(DiskD88Disk *disk, bool is_formatting)
{
	// グループ数
...
void DiskBasicTypeXDOS::CalcDiskFreeSize(bool wrote)
{
	...
	for(size_t pos = XDOS_FAT_START; pos < fatbuf->GetSize(); pos+=2) {
		...
		for(int bit = 0; bit < basic->GetSectorsPerTrackOnBasic() && gnum <= basic->GetFatEndGroup(); bit++) {
			...
			if (!used) {
				fat_availability.Item(gnum) = FAT_AVAIL_FREE;
				grps++;
			}
			gnum++;
		}
...
bool DiskBasicTypeXDOS::PrepareToSaveFile(wxInputStream &istream, int &file_size, const DiskBasicDirItem *pitem, DiskBasicDirItem *nitem, DiskBasicError &errinfo)
{
	// Chain用のセクタを確保する
